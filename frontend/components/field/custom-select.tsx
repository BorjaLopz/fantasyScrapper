...
export default function CustomSelect({
  ...
  const handleClickOutside = (e: any) => {
    // Closing the select's body if we click outside
    if (
  ...
  useEffect(() => {
    if (isOpen) {
      // If isOpen is true we need to "attach" the eventListener to clicking, beacuse we need to close the select's body if user clicks outside
      document.addEventListener("mousedown", handleClickOutside);
    } else {
      // If isOpen is false we need to remove the eventListener because that means the slect's body is already closed
  ...
  return (
    <Menu
      ...
      trigger={({ ...triggerProps }) => {
        return (
          <Button {...triggerProps}>
            <ButtonText>{currentFormation}</ButtonText>
          </Button>
        );
      }}
    >
